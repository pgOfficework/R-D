...
//		 checking search possibilities in policy collection
		AggregateIterable<Document> policyResult = policy.aggregate(Arrays.asList(
				new Document("$search",
						new Document("index", "policy").append("text", new Document("query", arg).append("path",
								Arrays.asList("policyNumber", "policyHolder.emails.email",
										"policyHolder.telephoneNumbers.telephoneNumber", "policyHolder.firstName"))))));
		...
		AggregateIterable<Document> claimResult = claim.aggregate(Arrays.asList(
				new Document("$search",
						new Document("index", "claim").append("text",

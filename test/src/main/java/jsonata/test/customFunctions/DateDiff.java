...
@Log4j2
public class DateDiff extends FunctionBase implements Function {
	...
	public JsonNode invoke(ExpressionsVisitor expressionVisitor, Function_callContext ctx) {
		...
		try {
			...
			if (argCount != 0) {
				...
				SimpleDateFormat format = new SimpleDateFormat(FunctionUtils.getValuesListExpression(expressionVisitor, ctx, 0).toString());//"yyyy-MM-dd'T'HH:mm:ss"
			...
			if (response.toString().contains("error")) {
				log.error("<" + DateDiff.class.getSimpleName()
